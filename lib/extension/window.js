...
export class WindowManager extends GObject.Object {
  ...
  showWindowBordersFor(metaWindow) {
    ...
    let borders = [];
    let focusBorderEnabled = this.ext.settings.get_boolean("focus-border-toggle");
    let inactiveBorderEnabled = this.ext.settings.get_boolean("inactive-border-toggle");
    ...
    borders.forEach((border) => {
      const borderThemeNode = border.get_theme_node();
      let borderOffset = borderThemeNode?.get_padding(St.Side.TOP) ?? 0;
      if (!borderOffset) {
        borderOffset = borderThemeNode?.get_margin(St.Side.TOP) ?? 0;
      }
      const borderInset = inset + borderOffset;
      const borderWidth = Math.max(1, rect.width + borderInset * 2);
      const borderHeight = Math.max(1, rect.height + borderInset * 2);

      border.set_size(borderWidth, borderHeight);
      border.set_position(rect.x - borderInset, rect.y - borderInset);

      let isFocused = false;
      try {
        isFocused = metaWindow.has_focus();
      } catch (e) {
        const focusedWindow = global.display.get_focus_window();
        isFocused = focusedWindow ? focusedWindow === metaWindow : metaWindow.appears_focused;
      }

      // show only focused border unless inactive borders are enabled
      if (!metaWindow.minimized && (isFocused || inactiveBorderEnabled)) {
        border.show();
      } else {
        border.hide();
      }
