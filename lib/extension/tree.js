...
export class Tree extends Node {
  ...
  processGap(node) {
    ...
    if (node.isWindow()) {
      let workArea = node.nodeValue.get_work_area_current_monitor();

      // Keep a smaller outer gap for windows that touch the top display edge.
      if (workArea && node.rect.y <= workArea.y) {
        topGap = Math.floor(gap / 2);
      }
    }

    if (nodeWidth > gap * 2 && nodeHeight > gap + topGap) {
      nodeX += gap;
      nodeY += topGap;
