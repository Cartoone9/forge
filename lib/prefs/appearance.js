...
export class AppearancePage extends PreferencesPage {
  ...
  _createColorOptionWidget(prefix) {
    ...
    const borderOffsetRow = new SpinButtonRow({
      ...
      width_chars: 2,
      xalign: 1,
      init: theme.removePx(theme.getCssProperty(selector, "margin").value ?? "0px"),
      onChange: (value) => {
        const px = theme.addPx(value);
        Logger.debug(`Setting border offset for selector: ${selector} ${px}`);
        theme.setCssProperty(selector, "margin", px);
      },
    });
    ...
    borderOffsetRow.add_suffix(
      new ResetButton({
        onReset: () => {
          const borderOffsetDefault = theme.defaultPalette[colorScheme].offset;
          theme.setCssProperty(selector, "margin", theme.addPx(borderOffsetDefault));
