...
export class AppearancePage extends PreferencesPage {
  ...
  static getCssSelectorAsMessage(selector) {
    switch (selector) {
      ...
      case ".window-unfocused":
        return _("Inactive window border");
    }
  }
  ...
  constructor({ settings, dir }) {
    ...
    this.add_group({
      ...
      children: [
        ...
        new SwitchRow({
          title: _("Border around inactive windows"),
          subtitle: _("Display borders for windows that are not focused"),
          settings,
          bind: "inactive-border-toggle",
        }),
        this._createInactiveBorderColorWidget(),
        new SwitchRow({
  ...
  _createInactiveBorderColorWidget() {
    const selector = ".window-unfocused";
    const theme = this.themeMgr;

    const inactiveBorderColorRow = new ColorRow({
      title: _("Inactive border color"),
      init: theme.getCssProperty(selector, "border-color").value,
      onChange: (rgbaString) => {
        Logger.debug(`Setting color for selector: ${selector} ${rgbaString}`);
        theme.setCssProperty(selector, "border-color", rgbaString);
      },
    });

    inactiveBorderColorRow.add_suffix(
      new ResetButton({
        onReset: () => {
          const defaultColor = "rgba(255, 255, 255, 0.5)";
          theme.setCssProperty(selector, "border-color", defaultColor);
          const rgba = new Gdk.RGBA();
          if (rgba.parse(defaultColor)) {
            inactiveBorderColorRow.colorButton.set_rgba(rgba);
          }
        },
      })
    );

    return inactiveBorderColorRow;
  }
  ...
  _createColorOptionWidget(prefix) {
    ...
    const borderOffsetRow = new SpinButtonRow({
      title: _("Border offset"),
      subtitle: _("Offsets the border by this many pixels"),
      range: [0, 32, 1],
      max_width_chars: 2,
      max_length: 2,
      width_chars: 2,
      xalign: 1,
      init: theme.removePx(theme.getCssProperty(selector, "padding").value ?? "0px"),
      onChange: (value) => {
        const px = theme.addPx(value);
        Logger.debug(`Setting border offset for selector: ${selector} ${px}`);
        theme.setCssProperty(selector, "padding", px);
      },
    });

    borderOffsetRow.add_suffix(
      new ResetButton({
        onReset: () => {
          const borderOffsetDefault = theme.defaultPalette[colorScheme].padding;
          theme.setCssProperty(selector, "padding", theme.addPx(borderOffsetDefault));
          borderOffsetRow.activatable_widget.value = borderOffsetDefault;
        },
      })
    );
