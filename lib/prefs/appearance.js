...
export class AppearancePage extends PreferencesPage {
  ...
  _createColorOptionWidget(prefix) {
    ...
    const borderSizeRow = new SpinButtonRow({
      ...
      width_chars: 2,
      xalign: 1,
      init: theme.removePx(theme.getCssProperty(selector, "border-width").value ?? "3px"),
      onChange: (value) => {
    ...
    borderSizeRow.add_suffix(
      new ResetButton({
        onReset: () => {
          const borderDefault =
            selector === ".window-unfocused"
              ? "3"
              : theme.defaultPalette[colorScheme]["border-width"];
          theme.setCssProperty(selector, "border-width", theme.addPx(borderDefault));
          borderSizeRow.activatable_widget.value = Number(borderDefault);
        },
      })
    );
    ...
    const updateCssColors = (rgbaString) => {
      ...
      if (rgba.parse(rgbaString)) {
        ...
        // The primary color updates the selected border:
    ...
    const borderOffsetRow = new SpinButtonRow({
      ...
      xalign: 1,
      init: theme.removePx(
        theme.getCssProperty(selector, "padding").value ??
          theme.getCssProperty(selector, "margin").value ??
          "0px"
      ),
      onChange: (value) => {
        const px = theme.addPx(value);
    ...
    borderOffsetRow.add_suffix(
      new ResetButton({
        onReset: () => {
          const borderOffsetDefault =
            selector === ".window-unfocused" ? "0" : theme.defaultPalette[colorScheme].offset;
          theme.setCssProperty(selector, "padding", theme.addPx(borderOffsetDefault));
          borderOffsetRow.activatable_widget.value = Number(borderOffsetDefault);
        },
      })
    );
        ...
        onReset: () => {
          const selectorColor =
            selector === ".window-unfocused"
              ...
              : theme.defaultPalette[colorScheme].color;
