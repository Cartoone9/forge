...
export class ThemeManagerBase extends GObject.Object {
   ...
   */
  getDefaults(color) {
    const borderSelector = `.window-${color}-border`;
    const borderOffset =
      this.getCssProperty(borderSelector, "padding").value ??
      this.getCssProperty(borderSelector, "margin").value ??
      "0px";
    ...
    return {
      ...
      offset: this.removePx(borderOffset),
    };
  }
  ...
  setCssProperty(selector, propertyName, propertyValue) {
    let cssRule = this.getCssRule(selector);
    ...
    if (!cssRule || !cssRule.declarations) {
      ...
      this.cssAst.stylesheet.rules.push(cssRule);
    }

    let cssProperty = cssRule.declarations.find((d) => d.property === propertyName);
    if (!cssProperty) {
      cssProperty = {
        type: "declaration",
